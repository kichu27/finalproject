...
function Analysis() {
  const [analyticsData, setAnalyticsData] = useState([]); // State to hold analytics data
const [d2 , setd2] = useState([])
   useEffect(() => {
    fetchAnalytics(); // Fetch data when component mounts
  }, []);

  const fetchAnalytics = async () => {
    try {
      const response = await fetch('/api/admin/analysis', { method: 'GET' }); // Fetch data from API
      if (!response.ok) {
        throw new Error('Failed to fetch analytics data');
      }
      const {data , data2} = await response.json(); // Parse response as JSON
